---
title: "Spring Boot 项目创建 Docker 镜像并运行应用"
  ...
  <plugins>
    <plugin>
      ...
      <configuration>
        ...
        <to>
            <image>zhijunio/${project.artifactId}</image>
            <tags>
                <tag>latest</tag>
...
1. 如果使用 Gradle，在 `build.gradle` 文件中添加 Jib Gradle 插件:
...
1. 运行 `mvn dockerfile:build` 命令构建镜像。
    ...
    <configuration>
        <images>
            <image>
                ...
                <name>zhijunio/${project.artifactId}:latest</name>
                <build>
...
<plugin>
    ...
    <configuration>
        <image>
            <name>zhijunio/${project.artifactId}</name>
