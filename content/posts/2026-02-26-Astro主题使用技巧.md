...
博客源代码：[zhijunio.github.io](https://github.com/zhijunio/zhijunio.github.io)
...
photosuite({
  scope: "#article",
  imageBase: "https://cos.zhijun.io/images",
  exif: {
      enabled: true,
      fields: [
        "Model",            // Camera Model
...
`https://cos.zhijun.io/images` 对应的是 obs 存储里面的 images 目录，这里我使用的是 Cloudflare R2 对象存储。cos.zhijun.io
是对象存储桶（桶名称为 cos）对应的自定义域名，该存储桶下目前主要有：

- **dist**：脚本库等资源
- **images**：博客配图
- **feeds**：聚合（Feeds）抓取的站点图标
...
在 frontmatter 中使用 `favicon` 字段，资源放在 `public/favicons/`。常用写法是**短文件名**（解析为 `/favicons/该文件`）；也可写完整根路径或外链。详见 `PostUtils.resolveFaviconRef` 与 `Card.astro` 中的 `buildListFaviconSrc`。
...
- favicon 为空：取 `/favicons/{首个分类}.svg`，无分类则用 `blank.svg`
- `http(s)://`：原样

config.ts 中定义了图片的 CDN 地址 `https://cos.zhijun.io`

```javascript title="config.ts"
imageConfig: {
    imagesUrl: "https://cos.zhijun.io",
}
```
