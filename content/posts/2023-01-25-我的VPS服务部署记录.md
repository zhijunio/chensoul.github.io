...
```bash
git config --global user.name zhijunio
git config --global user.email zhijun.lab@gmail.com
```
...
useradd -m -s /bin/bash zhijunio
usermod -aG wheel zhijunio
passwd zhijunio
...
之后可用 `su - zhijunio` 或 SSH 登录该用户，建议配置好 SSH 密钥后再考虑禁用 root 密码登录。
...
# 申请证书（支持通配符域名）
~/.acme.sh/acme.sh --issue --server letsencrypt --dns dns_cf -d zhijun.io -d "*.zhijun.io"

# 复制证书到 Nginx 目录
cp ~/.acme.sh/zhijun.io_ecc/{zhijun.io.cer,zhijun.io.key,fullchain.cer,ca.cer} /etc/nginx/ssl/

# 安装证书并设置自动续期
~/.acme.sh/acme.sh --installcert -d zhijun.io -d *.zhijun.io \
  --cert-file /etc/nginx/ssl/zhijun.io.cer \
  --key-file /etc/nginx/ssl/zhijun.io.key \
...
访问 <https://umami.zhijun.io/，默认用户名和密码为> `admin/umami`。登录之后，请立即修改密码，并添加要统计的网站。
...
访问 <https://memos.zhijun.io/> ，默认用户名和密码为 `admin/memos`。登录之后，请立即修改密码。
...
services:
  n8n:
    ...
    environment:
      ...
      - WEBHOOK_URL=https://n8n.zhijun.io/
      - N8N_HOST=n8n.zhijun.io
...
server {
    ...
    location / {
      ...
      proxy_set_header Origin "https://n8n.zhijun.io";
      access_log /var/log/nginx/n8n.log combined buffer=128k flush=5s;
    }
}
```

> **故障排查**：如果出现 `Origin header does NOT match the expected origin. (Origin: "n8n.zhijun.io", Expected: "127.0.0.1:5678")` 错误，需要在 nginx 配置里添加 `proxy_set_header Origin "https://n8n.zhijun.io";`
...
server {
    listen 80;
    listen [::]:80;
    server_name linkding.zhijun.io;
...
curl -sS -H "Authorization: Token ${LINKDING_TOKEN}" \
  "https://linkding.zhijun.io/api/bookmarks/?limit=100" \
...
services:
  artalk:
    ...
    environment:
      ...
      - ATK_SITE_URL=https://blog.zhijun.io
      - ATK_TRUSTED_DOMAINS=https://blog.zhijun.io http://localhost:1313
      - ATK_DB_TYPE=pgsql
      ...
      - ATK_ADMIN_NOTIFY_BARK_SERVER=https://bark.zhijun.io/AXWM5ZKnKa9Lp3rijBsVDm
...
浏览器输入 `http://artalk.zhijun.io` 进入 Artalk 后台登录界面。
...
访问 https://uptime.zhijun.io/ ，首次访问需要设置管理员账号。
...
1. 在 iOS 上安装 Bark 应用，添加私有服务器：`<http://bark.zhijun.io>

2. 测试推送功能：

```bash
# 替换 <device_key> 为你的设备密钥
curl -X "POST" "https://bark.zhijun.io/<device_key>" \
...
MIRROR="${MIRROR:-}"
GIT_USER="${GIT_USER:-zhijunio}"
GIT_EMAIL="${GIT_EMAIL:-zhijun.lab@gmail.com}"
...
| `CREATE_USER` | 创建并加入 wheel 的用户名，空则不创建 | `zhijunio` |
...
| `DOMAIN` | 主域名（通配符 *.$DOMAIN），空则不申请证书 | `zhijun.io` |
...
```bash
cd /opt
git clone git@github.com:zhijunio/vps-backup.git
...
MIRROR=aws-bos CREATE_USER=zhijunio DOMAIN=zhijun.io CF_Token="tEk7EtRfMHTC7kA9b_WwbJG-R4torcVzYhhBim3D" CF_Account_ID="2e1980a0ef43f57b920ea28cdf9d38d1" bash almalinux9-setup.sh
```
