---
title: "TBMQ 测试与源码分析"
date: 2024-04-28 08:00:00+08:00
slug: thingsboard-tbmq-test
categories: [ "tech" ]
tags: [ "thingsboard" ]
description: "记录 TBMQ 的基础测试流程与源码分析笔记，包括应用创建、消息收发验证以及关键实现逻辑的阅读过程。"
---
...
5. `initActor`方法用于初始化 Actor，它会将初始化任务分配给线程池执行。
6. `enqueue`方法用于将消息添加到邮箱中，它会根据消息的优先级将消息添加到相应的队列中。
7. `tryProcessQueue`方法用于处理消息，它会检查邮箱是否已准备好接收消息，是否在destroyInProgress中，以及是否还有消息需要处理。如果满足条件，它会调用`processMailbox`方法来处理消息。
8. `processMailbox`方法用于处理消息，它会从队列中取出消息，并调用`actor.process`方法来处理消息。
9. `getSelf`方法用于获取当前的ActorId。
10. `tell`方法用于发送消息给子Actor，它会将消息发送给系统。
11. `broadcastToChildren`方法用于向子Actor广播消息，它会将消息广播给系统中满足条件的子Actor。
12. `filterChildren`方法用于过滤子Actor，它会返回满足条件的子Actor列表。
13. `stop`方法用于停止Actor，它会将停止消息发送给系统。
14. `getOrCreateChildActor`方法用于创建或获取子Actor，它会根据条件来创建或获取子Actor。
15. `destroy`方法用于销毁Actor，它会设置`destroyInProgress`为true，然后销毁Actor。
16. 实现了`TbActorId`接口，该接口定义了Actor的唯一标识。
17. `tell`和`tellWithHighPriority`方法用于发送消息给子Actor，它会根据消息的优先级将消息添加到相应的队列中。
...
- tbmq.msg.retained
  - RetainedMsgConsumer
- tbmq.client.disconnect.zhijunio-mac.local
  - DisconnectClientCommandConsumer
- tbmq.client.session.event.response.zhijunio-mac.local
- tbmq.msg.downlink.basic.zhijunio-mac.local
