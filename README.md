...
| Pagefind                    | 本地全文搜索                                                                                   |
| COS (`cos.zhijun.io`)       | 图片与 `public/images`、`public/favicons`、`public/feeds` 等 CDN（生产环境经 `siteImageHref`） |
...
- **CI**：[`.github/workflows/fetch-feeds.yml`](./.github/workflows/fetch-feeds.yml) 调用 [`zhijunio/feeds-aggregator`](https://github.com/zhijunio/feeds-aggregator)，输入包括 `sources`、`output`、`favicon-dir`（如 `public/feeds`）、`favicon-public-prefix`（如 `/feeds`）；构建完成后将 `public/images`、`public/favicons`、`public/feeds` 同步至对象存储。
